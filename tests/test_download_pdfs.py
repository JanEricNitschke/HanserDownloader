...
@pytest.fixture
def soup_fixture() -> Iterator[BeautifulSoup]:
    """Fixture to provide a BeautifulSoup object for the test HTML file."""
...
def test_get_chapter_names_from_soup(soup_fixture: BeautifulSoup):
    ...
    result = get_chapter_names_from_soup(soup_fixture)
    assert result == expected_list


def test_format_chapter_link():
...
def test_get_chapter_pdf_links_from_soup(soup_fixture: BeautifulSoup):
    ...
    result = get_chapter_pdf_links_from_soup(soup_fixture)
...
def test_get_chapter(
    link_mock: MagicMock, name_mock: MagicMock, soup_fixture: BeautifulSoup
):
    """Tests get_chapter."""
    link_mock.side_effect = [["/1", "/2", "/3"], ["/1", "/2", "/3"]]
    name_mock.side_effect = [["a", "b", "c"], ["/1", "/2"]]
    result1 = get_chapters(soup_fixture)
    assert isinstance(result1, zip)
    assert list(result1) == [(1, "a", "/1"), (2, "b", "/2"), (3, "c", "/3")]
    result2 = get_chapters(soup_fixture)
...
def test_bad_response(get_mock: MagicMock):
...
@patch("download_pdfs.requests.get")
def test_good_response(get_mock: MagicMock):
